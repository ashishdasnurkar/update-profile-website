...
app.get('/', verifyInputToken, csrfProtection, (req, res) => {
  ...
  const requiredFields = req.tokenPayload[`${process.env.TOKEN_ISSUER}/claims/required_fields`];
...
function verifyInputToken(req, res, next) {
  ...
  try {
    req.tokenPayload = jwt.verify(req.query.token, process.env.TOKEN_SECRET, options);
  } catch (err) {
    return next(err);
  }
  return next();
